...
public class TransactionService(
    ...
    public async Task<Result<IReadOnlyList<TransactionModel>>> GetTransactionsByAccountAsync(string accountId, int page, int pageSize, CancellationToken token = default)
    {
        try
        {
            ...
            var transactions = await _db.Transactions
                .Where(t => t.Category!.AccountId == accountId)
    ...
    public async Task<Result<IReadOnlyList<TransactionModel>>> GetTransactionsByCategoryAsync(string categoryId, int page, int pageSize, CancellationToken token = default)
    {
        try
        {
            // Check if category exists
            var category = await _db.Categories.FindAsync([categoryId], token);
            if(category is null) return new TransactionServiceError.CategoryNotFound(categoryId);
    ...
    public async Task<Result> DeleteTransactionsAsync(string accountId, IReadOnlyList<string> transactionIds, CancellationToken token = default)
    {
        try
        {
            ...
            foreach (var transactionId in transactionIds)
            {
                ...
                if(transaction.Category.AccountId != accountId) 
                ...
                var category = await _db.Categories.FindAsync([transaction.CategoryId], token);
